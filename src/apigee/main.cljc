(ns apigee.main
  "Kotodama WASM entrypoint for the apigee clean-room actor (L4) — Clojure port.
  ...
  facts (`apigee.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "apigee")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"apigee.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "apigee-compat" :tier tier :entities entities} 200])
...
(defn start! [] :apigee-compat/ready)
